...
| A.11 | (user-requested "wow factor" #2) Cross-cluster strategic narrative — "what's the story across our top 5 problem clusters", genuine synthesis across clusters (shared system/vendor, overlapping timing, one root cause manifesting as several) instead of restating each in turn | 1 | **Done** — opt-in `?strategic_narrative=1` on Problem Clusters (project-wide top 5 by recurring_count, independent of the page's engine filter), ephemeral like A.7/A.9. Verified via Django test client on the real 11 Problem Candidate clusters: genuinely strategic-reading real result that connected 4 of the 5 clusters into one timing/integration theme and specifically flagged the DST-related cluster as a plausible link, not just restating each cluster; bridge-down showed a clear error. Confirmed live via browser |
| A.12 | (user-requested "wow factor" #3) Formal Problem Record draft — ITSM-format (Title/Description/Business Impact/Root Cause/Proposed Next Steps) instead of a loose paragraph, the artifact leadership recognizes as governance documentation, not a chatbot answer | 1 | **Done** — new `Cluster.problem_record_draft`/`problem_record_added_by`/`problem_record_added_at` fields (migration 0016), separate card on cluster detail (only shown for Problem Candidates), same draft-then-review-then-save UX as A.4 (generate renders into the textarea, does NOT auto-save — analyst still clicks "Save Problem Record"). Prompt enforces exactly 5 section headers and pulls in cost impact under Business Impact when configured. Verified via Django test client on a real Problem Candidate cluster: generate correctly left the DB untouched and produced a genuinely well-formed record with all 5 required sections, honestly noted "No specific dollar figure has been provided" rather than inventing one (cost isn't configured on this project) and correctly hedged the root cause as a hypothesis; save persisted it with real attribution; bridge-down left an already-saved value untouched. Confirmed live via browser |
| A.13 | (user-requested, 2026-08-01) One consistent gradient-hero style for every bridge result app-wide (previously: dashboard/login/about had the full F.1-F.3 hero treatment, everywhere else used a lighter violet-tinted `.ai-generated-panel`) + no width cap on the narrative text + richer, more detailed prompts everywhere that explicitly ask for quoted phrases/context, not generic restatement | 2 | **Done, with one honest verification gap.** `.ai-generated-panel`/`.ai-generated-label` redefined to the same teal→violet gradient as F.1-F.3 (`max-width: none`) — this single CSS change updated all 8 existing call sites at once (cluster Summary/Trend/Resolution-draft/Problem-Record, Duplicate explanation, Strategic narrative, Search synthesis, Log pattern narrative, Incident narrative); also converted the Problem Record *saved* display (previously plain, not gradient) to match. Six prompts enriched for length + explicit phrase-quoting (trend explanation, strategic narrative, search synthesis, duplicate explanation, pattern narrative, incident narrative) — deliberately left `ai_summary` terse (used as a compact inline label in tables elsewhere) and the Problem Record's 5-section structure as-is. Widened `Cluster.ai_trend_explanation`, `DuplicateCandidate.ai_explanation`, `LogPatternCluster.ai_narrative`, `LogPatternCorrelation.ai_incident_narrative` from `CharField(500/1000)` to `TextField` (migrations 0017, 0007) and removed the now-unneeded `[:500]`/`[:1000]` truncation, so the longer output doesn't get cut mid-sentence. **Verification gap**: the Copilot bridge itself was down at verification time (confirmed via a real connection-refused error, not a code issue) — couldn't generate fresh content to directly measure the new prompts' output. Verified everything up to that boundary instead: `manage.py check` clean, migrations applied without losing existing saved content, and live browser confirms the gradient/white-text/unconstrained-width styling renders correctly using the real content saved earlier this session (cluster 77's summary, the correlation group's incident narrative). Re-verify actual output length/quality once the bridge is back up. |
...
| F.3 | Login page redesign — split-panel (value prop + headline capabilities on one side, the sign-in form on the other), replacing the current bare centered box | 2 | **Done** — same gradient signature as F.1/F.2, three headline capabilities, links to the new `/about/` page. Stacks to a single column on narrow viewports (capability list hidden below 780px, form-first). Verified: gradient exact match at desktop width, capability list correctly visible at 1280px and correctly hidden+stacked at 375px mobile, "Learn more" link resolves to `/about/`, and the actual sign-in flow still works end-to-end through the redesigned form (real login → landed on Dashboard) |
| F.4 | (user-requested) Left nav regroup — 9 section labels (several single-item) collapsed to 6, grouped by workflow instead of implementation detail: Overview, Tickets, Clustering (merges the old Clustering Engines/Problem Insights/Cross-Project, with light `.nav-subdivider`s separating Run/Review/Configure), Lookup & Quality (merges Lookup Tools/Data Quality & Compliance — Possible Duplicates moved here, it's ticket hygiene not compliance), Log Monitoring, Reference | 1 | **Done** — every `active_nav` value/href/badge preserved exactly, pure reorder. Verified live: 6 sections × 18 items all present (incl. badges), active-state highlighting confirmed correct on 3 different pages after the regroup (Dashboard, Log Sources, Clustering Settings, Possible Duplicates). Found and fixed a real bug during verification: the new `.nav-subdivider` computed to 0px height (nav is a flex column, an explicit-height div with no `flex-shrink:0` gets squeezed to zero) — added `flex: 0 0 auto`, re-verified at the correct 1px |
| F.5 | (user-requested) Update `how_it_works.html` (Platform Guide) — it predated the entire Theme A Copilot Bridge suite, Theme E downloads, B.1 cost impact, and C.1 statistical trend test, and still claimed "no server-side LLM generation anywhere," which was no longer true | 1 | **Done** — rewrote pipeline step 7 (bridge is now automatic, manual copy/paste kept as fallback) and added step 8 (Export Anything); Feature Map gained an Overview tier and Bridge-chip badges for every new AI feature; added/rewrote table rows for Instant AI Briefing, Cost Impact, Statistical Trend Test, Resolution Draft, Problem Record, Strategic Narrative, Global Clustering, Search Synthesis, Duplicate Explanation, Pattern Narrative, Cross-Source Patterns, and Incident Narrative — every "Example" cell uses a real quote captured from actual generated output earlier in this session, none invented; rewrote Assumptions Made and Known Limitations to remove the false "no server-side LLM" claims and describe the real bridge model (no auth, not always-on, single-seat rate limits, license-scope caveat, non-deterministic + unreviewed-by-default for auto-saving features); added a "Delivered beyond the original scope" roadmap entry for the whole bridge suite, downloads, cost impact, trend test, and the portal/nav redesign. Verified: `manage.py check` clean, live page fetch confirms every new section/row/table entry present via direct text search |
